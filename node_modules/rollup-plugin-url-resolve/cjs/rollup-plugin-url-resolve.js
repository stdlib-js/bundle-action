...
var fs = require('fs');
var readData = require('data-uri-to-buffer');
...
var readData__default = /*#__PURE__*/_interopDefaultLegacy(readData);
...
function rewriteRelativeJavaScriptImports(base, code) {
  ...
  return babel__default['default'].transform(code, options).code;
}
...
async function loadURL(url, fetchOpts) {
  ...
  switch (url.protocol) {
    case 'data:':
      ...
      return readData__default['default'](url.href);
    case 'file:':
      return rewriteRelativeImports(
        url,
        mimeTypes__default['default'].lookup(url.href),
        fs.readFileSync(url).toString()
      );
    case 'http:':
    case 'https:':
      return fetch__default['default'](url.href, fetchOpts).then(res =>
        res.status === 404
          ? null
          : res.text().then(text => {
